/* ======================================================================
   ...
   Purpose:  To understand how read(), write() system calls work.
	...
	A. At a terminal, type
		$ gcc readWriteExample.c -o readWriteExample.exe 
		$ ./readWriteExample.exe
	----------------------------------------------------------------------
	...
	A. read()
		A.1 Task:
			...
			** File descriptor is a non-negative integer which acts as a
				handle for accessing a file. Each Linux process gets three 
				fd by default:
...
int main(){
	...
	/*
		Since strlen() exclude the terminating null byte ('\0'),
		we need to add 1 to indicate the actual size of a string.
	*/
	write(1, staticMsg, strlen(staticMsg) + 1);	// Write to stdout.
	...
	*/
	...
	/* 
		Since read() does not include any null byte ('\0') at the   
		end of the input stream, whereas a string needs '\0' byte at 
		the end to be a string, so we need to add '\0' at msgBuffer. 
		If we do not do it, we will get strange characters, when
		we will print message of msgBuffer.
	 */
	msgBuffer[rdSz] = '\0';

	strcpy(staticMsg, "Your written message: ");
	write(1, staticMsg, strlen(staticMsg) + 1); // Write to stdout.
	write(1, msgBuffer, strlen(msgBuffer) + 1); // Write to stdout.
